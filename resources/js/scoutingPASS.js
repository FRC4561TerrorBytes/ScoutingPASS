/* global QRCode, qr, screen, configData, teams, schedule, getTeams, getSchedule */
...
function addCounter (table, idx, name, data) {
  ...
  inp.setAttribute('style', 'background-color:black; color:white; border:none; text-align:center; text-transform:uppercase')
...
function addText (table, idx, name, data) {
  ...
  if (Object.prototype.hasOwnProperty.call(data, 'defaultValue')) {
...
function addElement (table, idx, name, data) {
  ...
  if (Object.prototype.hasOwnProperty.call(data, 'type')) {
    ...
    const err = (('defaultValue', 'No type specified'))
...
function configure () {
  const mydata = JSON.parse(configData)

  if (Object.prototype.hasOwnProperty.call(mydata, 'title')) {
    document.title = mydata.title
  }

  if (Object.prototype.hasOwnProperty.call(mydata, 'page_title')) {
    const elements = document.getElementsByClassName('page_title')
  ...
  let pmc = mydata.elements.prematch
  ...
  const ac = mydata.elements.auton
  const at = document.getElementById('auton_table')
  idx = 0
  ...
  const tc = mydata.elements.teleop
  const tt = document.getElementById('teleop_table')
  idx = 0
  ...
  const egc = mydata.elements.endgame
  const egt = document.getElementById('endgame_table')
  idx = 0
...
function validateData () {
  ...
  let rf = ''
  for (rf in requiredFieldsMap) {
    // Robot requires special (radio) validation
    if (rf === 'r') {
      if (!validateRobot()) {
        errStr += requiredFieldsMap[rf] + ', '
        ret = false
      }
    } else if (rf === 'l') {
      if (!validateLevel()) {
  ...
  if (ret === false) {
    window.alert('Enter all required values:\n' + errStr.trim().replace(/,\s*$/, ''))
  }
  return ret
}
...
function qrClear () { // eslint-disable-line
  qr.clear()
}

function clearForm () { // eslint-disable-line
  let match = 0
  let e = 0
...
function drawFields (name) {
  ...
  let f = ''
  for (f of fields) {
    const code = f.id.substring(7)
    const img = document.getElementById('img_' + code)
    const ctx = f.getContext('2d')
    ctx.clearRect(0, 0, f.width, f.height)
    ctx.drawImage(img, 0, 0, f.width, f.height)

    const xyStr = document.getElementById('XY_' + code).value
    if (JSON.stringify(xyStr).length > 2) {
      const pts = Array.from(JSON.parse(xyStr))
      let p = ''
      for (p of pts) {
        const coord = p.split(',')
...
function onFieldClick (event) { // eslint-disable-line
  ...
  const box = ((Math.ceil(event.offsetY / target.height * resH) - 1) * resL) + Math.ceil(event.offsetX / target.width * resL)
  const coords = event.offsetX + ',' + event.offsetY

  // Cumulating values
  const changingXY = document.getElementById('XY' + getIdBase(target.id))
  const changingInput = document.getElementById('input' + getIdBase(target.id))
...
function updateMatchStart (event) { // eslint-disable-line
  if ((getCurrentMatch() === '') || (!teams)) return
...
function onTeamnameChange (event) {
  const newNumber = document.getElementById('input_t').value
 ...
 */
function counter (element, step) { // eslint-disable-line
  const ctr = element.getElementsByClassName('counter')[0]
...
function undo (event) { // eslint-disable-line
  const undoID = event.firstChild
  // Getting rid of last value
  const changingXY = document.getElementById('XY' + getIdBase(undoID.id))
  const changingInput = document.getElementById('input' + getIdBase(undoID.id))
  let tempValue = Array.from(JSON.parse(changingXY.value))
  tempValue.pop()
  changingXY.value = JSON.stringify(tempValue)
...
window.onload = function () {
  configure()
  const ec = document.getElementById('input_e').value
  getTeams(ec)
  getSchedule(ec)
  this.drawFields()
}
